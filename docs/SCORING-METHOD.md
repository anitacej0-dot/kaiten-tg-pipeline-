...
> **Перенесено из инфлюенс-конвейера [kaiten-influence](https://github.com/cryptoyoginya/kaiten-influence).**
> Методика оценки не зависит от того, чей канал: те же 6 линз, те же зоны. Примеры и баллы
> в тексте относятся к блогерским сетам и оставлены как калибровочные образцы. Отличия для
> своего канала: линза Channel & Funnel читается как «уместность в ленте канала», а проверка
> голоса идёт против [voice-profile.md](../references/voice-profile.md), а не против автора-блогера.
...
> скоринг-отчётов проекта (`content/posts/<slug>/scoring/*.html`).
...
```mermaid
flowchart TD
    D1["Данные канала:<br/>5-10 постов автора ·<br/>комментарии · статистика"] --> GEN
...
1. **Генерация.** Агент пишет **5 вариантов** поста («заходов») под конкретный
   ...
   правило и чек-тесты: [DIVERSITY.md](DIVERSITY.md).
...
повторяются - это требование [правила разнообразия](DIVERSITY.md).

Исторический пример: [первый скоринг Светы](https://kaiten-influence.vercel.app/scoring/2026-week1-sveta-editorial.html) -
...
правит портреты. Портреты целиком: **[personas.md](../references/personas.md)**.

Поверх общей панели работают **сегментные панели** (`references/personas.md`) -
...
### На перспективу: инфо от сейлзов и других команд Кайтена

Этот канал данных пока не подключён - хотим начать собирать. Система
всеядна к полевым данным, и любой, кто разговаривает с клиентами,
сможет обогатить модель. Что будет особенно ценно:

- **Возражения из звонков** («а чем вы лучше X», «у нас безопасники
  против облака») → напитают Objections-линзу и реплики скептика;
...
- **Слова, которыми клиенты называют продукт и боль** → Message-Market Fit
  перестанет гадать про язык аудитории;
- **От саппорта**: за какими функциями приходят и на что жалуются →
  границы обещаний для Claims-линзы.

Механика будет той же, что для всего корпуса: скинуть текстом/скрином
агенту → разметка → нужный файл базы (панель сегмента, insights,
product-facts-заметки), данные с датой и источником.

### Что из этого автоматизирует парсер

План (спека осталась в инфлюенс-репо): парсер не только сайтов.
...
**[scoring-dataset.md](../references/scoring-dataset.md)** (каждый пример - с источником,
разметкой, баллами и уроком, который он дал рубрике; машиночитаемо -
[scoring-dataset.json](../references/scoring-dataset.json)). Параллельно я грузила в агента
