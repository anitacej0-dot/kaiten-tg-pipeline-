...
TYPE_LENGTH = {
    ...
    "обновление": (750, 1550),
    "кейс": (820, 1050),
    # Гипотеза по одному эталону #1377 (1060 просмотров, 19 реакций), а не измерение.
    # Пересчитать, когда наберётся 3-4 поста этого типа.
    "статья": (500, 1100),
}
...
TYPE_ALIASES = {
    ...
    "фича": "обновление",
    # «статья» больше не синоним кейса: у кейса есть герой-компания и подпись
    # «Опытом делится», у статьи-гайда героя нет, есть материал и карусель.
    "гайд": "статья",
    "разбор": "статья",
    "подборка": "статья",
    "эксперт": "статья",
...
PRODUCT_KW = [
    ...
    "workflow", "ограничен", "ресурсн", "телефони", "связк", "дополнен",
]
...
def check_type_rules(ptype, text, first):
    ...
    if ptype == "дайджест":
        ...
        if not re.search(r"(в стать[еи]|в карточках|подробнее|читайте)", low):
            fails.append("в статье-гайде нет отсылки к материалу или к карточкам")
        if not re.match(r"^\W*[^\w\s]", first):
            warns.append("первая строка статьи-гайда начинается без эмодзи")
    elif ptype == "кейс":
...
def check_variant(name, text, need_link, forced_type):
    ...
    # Кейс рассказывает про клиента, статья-гайд про класс инструментов вообще.
    # Продукт в них присутствует, но не разворачивается. Дайджест и обновление
    # обязаны дать продуктовую конкретику вглубь.
    per_para = 1 if ptype in ("кейс", "статья") else 2
